  ...
  is_valid(i: number): boolean {
    ...
    for (let pipe of this.pipes.pipes_info) {
      ...
      if ((bird_x - pipe.x) * (bird_x - pipe.x) + (bird_y - pipe.top_height) * (bird_y - pipe.top_height) <= 100) {
        if (this.birds[i].bird_info.alive) return false
      } else if ((bird_x - pipe.x) * (bird_x - pipe.x) + (bird_y - pipe.bottom_height) * (bird_y - pipe.bottom_height) <= 100) {
        if (this.birds[i].bird_info.alive) return false
      } else if (
        (bird_y <= pipe.top_height && pipe.x - bird_x <= 10) ||
        (bird_y >= window.innerHeight - pipe.bottom_height && pipe.x - bird_x <= 10)
      ) {
        if (this.birds[i].bird_info.alive) return false
      } else if (
        bird_x >= pipe.x &&
        bird_x <= pipe.x + 40 &&
        (bird_y - 10 <= pipe.top_height || bird_y + 10 >= window.innerHeight - pipe.bottom_height)
      ) {
        if (this.birds[i].bird_info.alive) return false
      }
      return true
